...
func UpdateStock(w http.ResponseWriter, r *http.Request) {
	...
	if err = json.NewDecoder(r.Body).Decode(&stock); err != nil {
		log.Fatalf("unable to decode the req %v", err)
	}
...
func DeleteStock(w http.ResponseWriter, r *http.Request) {
	...
	detetedRows := deleteStock(int64(id))
	msg := fmt.Sprintf("stock deleted successfully. Total rows/records %v", detetedRows)
...
func insertStock(stock models.Stock) int64 {
	...
	err := db.QueryRow(sqlStatement, stock.Name, stock.Company).Scan(&id)
